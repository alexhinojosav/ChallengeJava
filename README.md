...
Please install netbeans or eclipse IDE
Open ChallengeJava Proyect
Right click on each test(you can select both tests)
Then click on run tests
Test reports are on  the follow path ChallengeJava\target\surefire-reports

Note: before run tests, the application needs to be running with the yarn start command.
